...
train_envs = ts.env.DummyVectorEnv([lambda: gym.make('numberx-v0', device=device) for _ in range(8)])
test_envs = ts.env.DummyVectorEnv([lambda: gym.make('numberx-v0', device=device) for _ in range(16)])
